...
export class PbsStationsComponent implements OnInit, OnDestroy {
  ...
  /**
   * True only until the very first load attempt (success or failure)
   * finishes. Used to show a full-panel loader instead of the "no data"
   * / error state on first paint, so the user never sees an empty or
   * gated screen while the initial fetch is still in flight.
   */
  isInitialLoad = true;
  ...
  ngOnInit(): void {
    ...
    // Kick off loading immediately — no manual "Login" click required.
    this.loadData();
  }
  ...
  /** Manual retry action shown in the error banner (replaces the old "Login" button). */
  retry(): void {
  ...
  logout(): void {
    ...
    // Immediately re-authenticate rather than showing a gated login screen.
